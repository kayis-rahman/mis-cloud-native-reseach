...
# Validation script for GCP deployment (Terraform + GKE + Helm)
# Usage: scripts/validate_gcp_deployment.sh [--project <PROJECT_ID>] [--location <REGION>] [--cluster <CLUSTER_NAME>] [--namespace <NAMESPACE>]
# If values are not provided, the script will try to read them from Terraform outputs.
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --namespace)
      NAMESPACE="$2"; shift 2;;
    -h|--help)
      echo "Usage: $0 [--project <PROJECT_ID>] [--location <REGION>] [--cluster <CLUSTER_NAME>] [--namespace <NAMESPACE>]"; exit 0;;
...
if [[ -z "$PROJECT_ID" ]]; then PROJECT_ID="$ACTIVE_PROJECT"; fi
if [[ -z "$PROJECT_ID" ]]; then
  fail "GCP project not set. Set with: gcloud config set project <PROJECT_ID> or pass --project"; exit 1
fi
...
if [[ -f "terraform/outputs.tf" ]]; then
  ...
  # If state is present locally, read outputs
  if terraform output -json >/dev/null 2>&1; then
    ...
    pass "Terraform outputs: cluster=$CLUSTER_NAME location=$LOCATION"
  else
...
if [[ -z "$CLUSTER_NAME" ]]; then
  warn "Cluster name not provided and not found in terraform outputs. Defaulting to mis-cloud-native-gke"
  CLUSTER_NAME="mis-cloud-native-gke"
fi
if [[ -z "$LOCATION" ]]; then
  warn "Cluster location not provided and not found in terraform outputs. Defaulting to us-central1"
...
step "Readiness probes summary for known services"
for svc in cart identity order payment product; do
  if kubectl get deploy -n "$NAMESPACE" | grep -q "$svc"; then
    kubectl rollout status deploy -n "$NAMESPACE" | grep "$svc" || true
  fi
done
